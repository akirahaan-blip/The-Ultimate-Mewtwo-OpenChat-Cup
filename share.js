/**
 * The Ultimate Mewtwo OpenChat Cup - X投稿用の書き出し
 *
 * ・投稿文をクリップボードへコピー
 ...
 */
import { SUB_SKILLS, INGREDIENTS, INGREDIENT_SCORES, NATURES, NATURE_SCORES } from './scoring.js?v=33';
import { INGREDIENT_LIST } from './ingredients-setting.js?v=33';
...
export function buildPostText(state, result) {
  ...
  if (ing && state.ingredientPattern !== 'OTHER') {
    lines.push(`${INGREDIENT_LIST[0].emoji}${state.ingredientPattern}（${ingShort}）`);
  }
...
export async function buildScoreCanvas(state, result) {
  ...
  grad.addColorStop(0, '#9b6bd6');
  grad.addColorStop(0.5, '#b98ae0');
  ...
  // ---- ヘッダー ----
  let x = PAD;
  const mascot = await loadImage('mewtwo.png');
  if (mascot) {
    const s = 92;
    ctx.fillStyle = '#f2ebfd';
  ...
  if (tagRows.length) {
    ...
    tagRows.forEach((row, ri) => {
      ...
      for (const tag of row) {
        ctx.fillStyle = tag.gold ? '#fff9e6' : '#f4edfd';
        roundRect(ctx, tx, ty, tag.w, 42, 8);
        ctx.fill();
        ctx.strokeStyle = tag.gold ? '#ffe082' : '#dcc8f5';
        ctx.lineWidth = 2;
        roundRect(ctx, tx, ty, tag.w, 42, 8);
        ctx.stroke();
        ctx.fillStyle = tag.gold ? '#b78103' : '#6a2fa0';
  ...
  ctx.textAlign = 'left';
  ctx.fillText('The Ultimate Mewtwo OpenChat Cup', PAD, y + 46);
  ctx.textAlign = 'right';
